...
@php
    ...
    $readingTime = $post->reading_time ?? max(1, (int) ceil(str_word_count(strip_tags($post->body)) / 200));
...
<article class="min-w-0">
    ...
    <div class="mb-8 flex flex-wrap items-center gap-3 text-sm">
        @if ($post->badge)
            <div class="rounded-full bg-brand px-3 py-1 font-medium text-brand-foreground">
                {{ $post->badge }}
            </div>
        @endif
    ...
    @if ($post->image)
        <figure class="mt-8 overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
            <img src="{{ asset('storage/' . $post->image) }}" alt="{{ $post->image_alt ?? $post->title }}"
                width="1200" height="800" class="h-auto w-full object-cover" loading="eager" />
            @if ($post->image_caption)
                <figcaption
                    class="border-t border-border bg-secondary px-5 py-3 text-center text-sm text-muted-foreground">
                    {{ $post->image_caption }}
                </figcaption>
            @endif
        </figure>

...
<div class="flex items-center justify-between mb-6">
    <h3 class="text-base font-medium text-gray-900">Tum Kategoriler</h3>
    ...
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-gray-100">
            ...
            <tr class="hover:bg-gray-50">
                <td class="px-4 py-3 font-medium text-gray-900">{{ $category->getTranslation('name', 'tr') }}</td>
                <td class="px-4 py-3 text-gray-600">{{ $category->getTranslation('name', 'en') }}</td>
                <td class="px-4 py-3 text-gray-500">{{ $category->slug }}</td>
                <td class="px-4 py-3">
                    <span class="px-2 py-1 rounded-full text-xs {{ $category->is_active ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700' }}">
